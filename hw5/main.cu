...
__global__ void kunion(int *output,int *a,int length,int size,int ids,int blocknum,int threadnum);
...
__global__ void kunion(int *output,int *a,int length,int size,int ids,int blocknum,int threadnum)
{
	...
	for(i=(ids+1)*length+bid * threadnum + tid;i<length*size;i+=BLOCKNUM * THREADNUM)
	{
		int j = i % length;
		output[i]=a[i]&a[ids*length+j];
	}
	__syncthreads();
...
void mineGPU(EClass *eClass, int minSup, int* index, int length){
	...
	countforgpu+=1;
	//printf("gpu here %d =%d \n",countforgpu,size);
	int *cudaforadb; // adb is for *a
	int *cudaforoutdb;
	cudaMalloc((void**) &cudaforadb,(size)*sizeof(int)*length);
	cudaMalloc((void**) &cudaforoutdb,(size)*sizeof(int)*length);
	for(int ide=0;ide<size;ide++)
	{
		cudaMemcpy(cudaforadb+ide*length, eClass->items[ide].db, sizeof(int)*length, cudaMemcpyHostToDevice);
	}
	...
	for (int i = 0; i < size; i++){
		...
		//cout<<"i here "<<i<<endl;
		 // bdb is for *b
		//outdb is for answer;
		int *tempout    = new int[size*length];

		//cout<<"alive and i = "<<i<<endl;
		//cudaMalloc((void**) &cudaforbdb,(size)*sizeof(int)*length);
		//cudaMalloc((void**) &cudaforoutdb,(size)*sizeof(int)*length);
		
		/*
		int j=i+1;
		for(int ide2=0;ide2<size-i-1;ide2++)
		{
			cudaMemcpy(cudaforbdb+ide2*length, eClass->items[j].db, sizeof(int)*length, cudaMemcpyHostToDevice);
			j=j+1;
		}
		*/
		kunion<<<BLOCKNUM,THREADNUM>>>(cudaforoutdb,cudaforadb,length,size,i,BLOCKNUM,THREADNUM);
		
		cudaDeviceSynchronize();
		cudaMemcpy(tempout+(i+1)*length, cudaforoutdb+(i+1)*length, (size-i-1)*sizeof(int)*length, cudaMemcpyDeviceToHost);
		...
		for(int k=(i+1);k<size;k++)
		{
			...
			support=0;
			memcpy(temp2,&tempout[k*length],length*sizeof(int));
			
			for(int l=0;l<length;l++)
			{

				//if(tempout[k*length+l]!=0)
				//	cout<<"temp here"<<tempout[k*length+l]<<endl;
				support += NumberOfSetBits(temp2[l]);
				//if (support!=0)
				//	cout<<"it's support="<<support<<endl;
			}
			
			//cout<<"true support"<<support<<endl;
			
			if (support >= minSup)
			{
				//cout<<"this is gpu support"<<support<<endl;
				//cout<<"id= "<<eClass->items[k].id<<endl;
				children->items.push_back(Item(eClass->items[k].id, temp2, support));
			}
		...
		/*
		for(int ide=i+1;ide<size;ide++)
		{
			cudaMemcpy(cudaforbdb+(ide-1)*length,eclass->items[ide].db,sizeof(int)*length,cudaMemcpyHostToDevice);
		}
		*/
		//cudaFree(cudaforadb);
		//cudaFree(cudaforoutdb);
		delete[] tempout;
	...
	/*
	./fim.out retail.txt 0.001 outfile
	...
	for (int i = 0; i < size; i++){
		...
		children->parents.push_back(eClass->items[i].id);
		int *cudaforadb; // adb is for *a
		cudaMalloc((void**) &cudaforadb,sizeof(int)*length);
		int *a = eClass->items[i].db;
		cudaMemcpy(cudaforadb, a, sizeof(int)*length, cudaMemcpyHostToDevice);
		for (int j = i + 1; j < size; j++){
			int * temp = new int[length];
			int *b = eClass->items[j].db;
			int support = 0;
			int *cudaforbdb; // bdb is for *b
			int *cudaforoutdb;//outdb is for answer;
			int *tempout    = new int[length];
			cudaMalloc((void**) &cudaforbdb,sizeof(int)*length);
			cudaMalloc((void**) &cudaforoutdb,sizeof(int)*length);
			
			cudaMemcpy(cudaforbdb, b, sizeof(int)*length, cudaMemcpyHostToDevice);
			kunion<<<BLOCKNUM,THREADNUM>>>(cudaforoutdb,cudaforadb,cudaforbdb,length,BLOCKNUM,THREADNUM);
			cudaMemcpy(tempout, cudaforoutdb, sizeof(int)*length, cudaMemcpyDeviceToHost);
			
			for (int k = 0; k < length; k++){
				//temp[k] = a[k] & b[k];
				support += NumberOfSetBits(tempout[k]);
			}
	
			cudaFree(cudaforbdb);
			cudaFree(cudaforoutdb);
			if (support >= minSup){
				children->items.push_back(Item(eClass->items[j].id, tempout, support));
			}
	...
	for (int i = 0; i < size; i++){
		...
		for (int j = i + 1; j < size; j++){
			...
			for (int k = 0; k < length; k++){
				...
				//cout<<"tempk="<<support<<endl;
			}
			if (support >= minSup){
				//cout<<"this is cpu support"<<support<<endl;
				//cout<<"cpu id = "<<eClass->items[j].id<<endl;
